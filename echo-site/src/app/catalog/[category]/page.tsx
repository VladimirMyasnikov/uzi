...
import { getProductsByCategory, getCatalogCategories } from "@/lib/catalog-loader";
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16 space-y-10">
      <div className="space-y-2">
        ...
        <p className="text-slate-600">Ассортимент категории. Описание уточняется при запросе.</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {products.map((prod) => (
              <Link
                ...
                href={`/catalog/${encodeURIComponent(category.slug)}/${encodeURIComponent(prod.productSlug)}`}
                className="card-panel block overflow-hidden rounded-2xl p-0 transition hover:-translate-y-1 hover:shadow-md"
              >
                ...
                <div className="p-5 space-y-2">
                  ...
                  <div className="text-lg font-semibold text-slate-900">{prod.title}</div>
                  {prod.status ? (
                    <div className="text-sm text-slate-600">Статус: {prod.status}</div>
                  ) : null}
                  <div className="text-sm font-semibold text-slate-900 hover:text-slate-700">
                    Подробнее →
                  </div>
                </div>
              </Link>
